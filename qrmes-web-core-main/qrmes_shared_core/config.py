...
LOCAL_APP_WEB_CONFIG_FILE = Path(__file__).resolve().parents[1] / 'app_web' / 'webdav_config.json'
...
def _apply_env_overrides(config_data: dict) -> dict:
    overrides = {
        ...
        'kingdee_local_db_api_base': _env_text('KINGDEE_LOCAL_DB_API_BASE'),
        'mes_remote_ssh_host': _env_text('MES_REMOTE_SSH_HOST'),
        'mes_remote_ssh_username': _env_text('MES_REMOTE_SSH_USERNAME'),
        'mes_remote_ssh_password': _env_text('MES_REMOTE_SSH_PASSWORD'),
        'mes_remote_root': _env_text('MES_REMOTE_ROOT'),
        'mes_remote_snapshot_dir': _env_text('MES_REMOTE_SNAPSHOT_DIR'),
        ...
        'ai_input_price_per_1k_usd': _env_text('AI_INPUT_PRICE_PER_1K_USD', 'QWEN_INPUT_PRICE_PER_1K_USD'),
        'ai_output_price_per_1k_usd': _env_text('AI_OUTPUT_PRICE_PER_1K_USD', 'QWEN_OUTPUT_PRICE_PER_1K_USD'),
    }

    bool_overrides = {
        'kingdee_workhour_enabled': _env_bool('KINGDEE_WORKHOUR_ENABLED'),
        'kingdee_verify_ssl': _env_bool('KINGDEE_VERIFY_SSL'),
        'mes_remote_ssh_enabled': _env_bool('MES_REMOTE_SSH_ENABLED'),
    }

    int_overrides = {
        'kingdee_lcid': _env_int('KINGDEE_LCID'),
        'kingdee_timeout_secs': _env_int('KINGDEE_TIMEOUT_SECS', 'KINGDEE_TIMEOUT_SECONDS'),
        'kingdee_local_db_api_timeout_secs': _env_int('KINGDEE_LOCAL_DB_API_TIMEOUT_SECS'),
        'mes_remote_ssh_port': _env_int('MES_REMOTE_SSH_PORT'),
        'mes_remote_snapshot_ttl_seconds': _env_int('MES_REMOTE_SNAPSHOT_TTL_SECONDS'),
...
def _merge_missing_legacy_config(config_data: dict) -> dict:
    """Fill missing runtime values from legacy app_web config files.

    The split v3 runtime imports this package-level config file, while older
    deployments and some UI edits kept secrets in app_web/webdav_config.json.
    Preserve existing runtime values, but migrate non-empty legacy values for
    keys that are absent or blank here.
    """
    merged = dict(config_data or {})
    candidates = [LOCAL_APP_WEB_CONFIG_FILE, *RUNTIME_CONFIG_CANDIDATES]
    seen = set()
    for candidate in candidates:
        try:
            ...
            resolved = candidate
        if resolved in seen or resolved == CONFIG_FILE.resolve():
            continue
        seen.add(resolved)
...
DEFAULT_CONFIG = {
    ...
    "kingdee_app_id": "",
    "kingdee_app_secret": "",
    "kingdee_local_db_api_base": "http://127.0.0.1:9010",
    "kingdee_local_db_api_timeout_secs": 30,
    "kingdee_lcid": 2052,
    ...
    "ai_input_price_per_1k_usd": "0.008",
    "ai_output_price_per_1k_usd": "0.024",
}
...
class Config:
        ...
        if CONFIG_FILE.exists():
            try:
                with open(CONFIG_FILE, 'r', encoding='utf-8') as f:
                    self._config = _apply_env_overrides(_merge_missing_legacy_config(json.load(f)))
                print(f"[Config] Loaded configuration from {CONFIG_FILE}")
            except Exception as e:
                print(f"[Config] Error loading config: {e}, using defaults")
                self._config = _apply_env_overrides(_merge_missing_legacy_config(DEFAULT_CONFIG.copy()))
        else:
    ...
    def _save_config(self):
        """保存配置"""
        target_candidates = [CONFIG_FILE, LOCAL_APP_WEB_CONFIG_FILE, SOURCE_CONFIG_FILE]
        runtime_target = None
