...
rows_data = [
    ...
    ("六角法兰面螺栓 加大系列 B级","T19020414.A1","GB/T 5789（ M5×20/10.9级/达克罗）","","","",5.0),
    ("六角法兰面螺栓","T19020303.A1","GB/T 5787-2000（M8X20/8.8级/达克罗/带齿）","","","",2.0),
    ...
    ("弹簧管卡","W19010001.A0","锰钢，WN27","",0.02,"",2.0),
...
with tempfile.TemporaryDirectory(prefix="finance_test_") as temp_dir:
    ...
    for av in [1000, 10000, 50000]:
        ...
        for r in detail_rows:
            name = r.get("物料", "")
            if name in ("定子总成", "定子组件", "机壳", "电机轴", "电控外壳", "前端盖", "转子挡板", "转子轴套"):
                print(f"  {name}: 基准={r.get('基准总价')} 保守={r.get('保守总价')} 激进={r.get('激进总价')}")
